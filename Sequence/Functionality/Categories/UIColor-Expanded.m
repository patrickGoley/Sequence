...
+ (UIColor *)colorWithString:(NSString *)stringToConvert {
    ...
    NSUInteger i = 0;
	float colorFloat = c[i++];
	
	if (![scanner scanFloat:&colorFloat])
		return nil;
...
+ (UIColor *)colorWithWhiteRGB:(NSInteger)white alpha:(CGFloat)alpha {
    return [UIColor colorWithRed:white / 255.0f
                           green:white / 255.0f
                            blue:white / 255.0f
                           alpha:alpha];
}
...
+ (UIColor *)colorWithHexString:(NSString *)stringToConvert {
    ...
    if (!stringToConvert) return [UIColor blackColor];
